...
const BuyData = () => {
  ...
  const [selectedDataPlan, setSelectedDataPlan] = useState<DataPlans | null>(
    null
  );
  ...
  const { data: fetchDataPlans } = useFetchDataPlansQuery({
    token: storedToken,
  });
  ...
  const handleNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!phoneNumberPattern.test(cleanValue)) {
      ...
      setNumberError(
        "Please enter a valid phone number in either format:\n+234XXXXXXXXXX\nor\n234XXXXXXXXXX"
      );
    } else {
    ...
    <div>
      {isMobileView ? (
        ...
        <div className="min-h-screen w-full bg-black pt-7 px-16 max-sm:px-7 flex flex-col justify-between">
          ...
          <p className="text-white font-[400] text-base font-poppins mt-10">
            Select any network
          </p>
          <div className="flex justify-between items-center py-3 mt-2 ">
            {fetchNetworks &&
              fetchNetworks.data.map((network: Network, index: number) => {
                return (
                  <div
          ...
          <form className="mt-10 flex-grow flex flex-col justify-between pb-20">
            <div>
              <div className="">
                <p className="text-white font-[500] text-base font-poppins mb-5">
                  Phone number
                </p>
                <input
                  type="number"
                ...
                {fetchDataPlans &&
                  ...
                  Object.entries(
                    fetchDataPlans.data
                      .filter(
                        (dataPlan: DataPlans) =>
                          dataPlan.network === selectedNetwork.networkname
                      )
                      .reduce(
                        (
                          acc: Record<string, DataPlans[]>,
                          dataPlan: DataPlans
                        ) => {
                          const planType = dataPlan.planType;
                          if (!acc[planType]) {
                    ...
                    ([planType, plans]) =>
                      ...
                      planType !== "DATA COUPONS" && (
                        <div key={planType} className=" mt-10 gap-4">
                          ...
                          <div className="flex justify-between flex-wrap items-center gap-4 mt-2">
                            {(plans as DataPlans[]).map(
                              (dataPlan: DataPlans, index: number) => {
                                const isSelected =
                                  selectedDataPlan &&
                                  selectedDataPlan.data_id === dataPlan.data_id;
                                return (
                                  <div
                                    key={index}
                                    className={`flex flex-col rounded-xl border h-28 w-[30%] justify-center items-center gap-2 cursor-pointer ${
                                      isSelected
      ...
      {paymentSuccessfulModal && (
        <div
          className="fixed  inset-0 mx-2 bg-[#1E1E1E] h-[450px] py-5 px-10 flex z-50 justify-between m-auto rounded-[15px] flex-col"
          style={{
        ...
        >
          ...
          <p className="text-[#FFFFFF6B] font-[300] font-poppins text-base text-center">
