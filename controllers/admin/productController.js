...
export const updateUtilityDiscount = async (req, res, next) => {
  try {
    const { percentage } = req.body;
    const adminUserId = req.user.id;

    if (percentage === undefined || percentage === null) {
      throw new ApiError(400, false, 'Percentage value is required');
    }

    const numericPercentage = Number(percentage);
    if (Number.isNaN(numericPercentage)) {
      throw new ApiError(400, false, 'Percentage must be a number');
    }
    ...
    const updatedSetting = await Setting.updateSetting(
      'UTILITY_DISCOUNT_PERCENT',
      numericPercentage,
      adminUserId
    );

    await logUserActivity(adminUserId, 'UPDATE_UTILITY_DISCOUNT', {
      oldPercentage: updatedSetting.previousValue?.value,
      newPercentage: numericPercentage,
    });

    return res.status(200).json({
      success: true,
      message: 'Utility Discount updated successfully',
      data: {
    ...
    console.error('Failed to update utility discount', error);
    next(error);
  }
};
